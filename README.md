...
- **Station Map** — sampling locations over an offline satellite basemap (Web-Mercator), colour-coded by any bottle variable. The geolocated **live sonde track** can be overlaid and coloured by any YSI parameter (its own colourbar), so you can see e.g. the conductivity gradient laid over the station grid. A **Stations** overlay (from the canonical `CTD_LOG.csv` cast log) marks every cast — named **FULL stations** as labelled gold diamonds, **CTD** transect casts as green dots, **SMBOAT** (small-boat) casts as purple triangles; hover for the cast notes.
